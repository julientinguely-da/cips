...
A delegate performs critical network-related tasks, such as for example advancing mining rounds or executing proposals when the required 2/3 voting thresholds are reached.
...
Our approach is unlikely to degrade performance in practice, as it introduces a minimal overhead in SV app automation while effectively improving fault tolerance.
Most of the tasks succeed on a first attempt, making the backoff mechanism effectively inert. 
Empirical benchmarks under expected workloads reinforce the claim that delays avoid concurrent execution for a same task.
Moreover, the simplicity of the mechanism keeps the system maintainable and avoids the complexity costs associated with more deterministic coordination strategies.
